/**
 * @file SpringReverb_Arduino.ino
 * @author Piotr Zapart
 * @brief Example project for the Stereo Spring Reverb component
 * 			Standard (int16_t) Teensy Audio library compatible
 * 		required libraries: 
...
AudioEffectSpringReverb_i16		reverb;
...
void cb_ControlChange(uint8_t channel, uint8_t control, uint8_t value)
{
	...
	switch(control)
	{
		...
		case MIDI_CC_REVERB_MIX:
			reverb.mix(valuef);
			break;
		case MIDI_CC_REVERB_BASS_CUT:
			reverb.bass_cut(valuef);
			break;
		case MIDI_CC_REVERB_TREBLE_CUT:
			reverb.treble_cut(valuef);
			break;
		default:    break;
	}	
}
